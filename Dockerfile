...
# 기존 yara, libyara-dev 제거, 컴파일 도구 추가 (make, automake, libtool 등)
RUN apt-get update && \
...
RUN curl -L -o yara-${YARA_VERSION}.tar.gz "https://github.com/VirusTotal/yara/archive/refs/tags/v${YARA_VERSION}.tar.gz" && \
    ...
    ./bootstrap.sh && \
    ./configure --enable-magic --enable-dotnet --enable-crypto && \
    make && \
    make install && \
    cd .. && \
...
# YARA가 기본적으로 /usr/local/lib 에 설치되므로 해당 경로 추가
ENV LD_LIBRARY_PATH=/usr/local/lib:$LD_LIBRARY_PATH

# 7. requirements.txt 복사 및 파이썬 패키지 설치
COPY requirements.txt .
# requirements.txt 에 yara-python==4.2.3 지정 권장 (컴파일한 C 버전과 일치)
