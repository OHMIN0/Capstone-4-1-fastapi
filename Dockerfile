    ...
    # yara, libyara-dev, build-essential, python3-dev 설치 유지
    RUN apt-get update && \
    ...
    #     cp /usr/lib/x86_64-linux-gnu/libyara.so /opt/venv/lib/libyara.so # 이 줄 제거

    # 6. requirements.txt 복사 및 파이썬 패키지 설치
    COPY requirements.txt .
    # requirements.txt 에서 yara-python 버전을 C 라이브러리와 호환되게 수정해야 함 (예: 4.2.0)
